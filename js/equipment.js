function initialiseEquipmentModule() {
...
// Build the gallery from records that genuinely describe equipment, software or infrastructure.
...
// Exclude events, people and places before checking for technical record types and keywords.
...
// Prepare a consistent title, ID, image and search value for each gallery record
...
// One record may belong to several categories when it represents more than one kind of technology
...
// Use a direct image when available, then look through the record’s linked media
...
// Create an image card for each result, with a text card as the fallback
...
// Open the shared record drawer through whichever drawer entry point is available
...
// Connect the opening button, category filters and live search, then draw the gallery
...
window.initialiseEquipmentModule = initialiseEquipmentModule;