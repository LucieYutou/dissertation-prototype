...
// Place names from different datasets are brought into a consistent form before the maps are drawn.
...
// These small utilities handle labels, tooltips, colours and SVG elements used across the page.
function peopleRows() {
...
// The data is filtered and grouped here before it reaches any chart.
...
function getYearsWithActualCategoryData(population, categoryType, measure, options = {}) {
  ...
  getRows({
    ...
    // A year is kept only when at least one visible category has a value above zero.
    // Staff nationality begins with 2018/19.
...
// Shared axes, legends and empty states keep the different charts visually consistent.
...
// Student and staff totals are brought together on one time axis for the opening chart.
...
// The trend and distribution charts update together when the year or category changes
...
function renderPieChart(options) {
  ...
  // Very small slices are left out of the All view so labels do not appear as 0.0%
...
// The programme ranking determines which ten programmes appear in the trend chart
...
function renderProgrammeMultiTrend() {
  ...
  if (title) title.textContent = peopleState.selectedProgramme || "Top 10 programmes";
...
// The map, region trend and lower comparison panel all respond to the same place selection
...
// This function can keep the four UK regions separate or add them to the United Kingdom total
...
// One change to the student controls refreshes degree level, gender, programmes and domicile together.
...
// Changing the staff year or measure updates the role, gender and nationality charts.
...
// A person card opens the same record drawer used elsewhere in the prototype.
...
// The page starts only after the People dataset is available.
