function initialiseTimelineModule() {
// Curated mode uses this smaller set of labelled milestones
...
// Detailed mode adds more selected records to the curated set
...
// This list allows the Build the School filter to match the guided route exactly
...
// Each view keeps its own search, filter, selection and scroll position
...
// Dates, titles and tags are normalised here before records are filtered or drawn.
...
function openTimelineDrawer(itemId, config = null) {
  ...
  // Selecting the open record again clears the highlight and closes the drawer.
...
// Mode, filter, search and year controls update the view currently in use
...
function renderTimelineModes() {
  ...
  button.addEventListener("click", () => {
    ...
    // A different mode starts from its own default state.
...
// The small overview marks the records and the part of the timeline currently on screen
...
// urated mode shows individual milestones, while Detailed mode groups records by year
...
function assignDetailedClusterLayout(groups) {
  ...
  const layouts = groups.map(([year, groupEntries]) => {
    ...
    // Circle size reflects record count but stays within a readable range.
  ...
  // Neighbouring years are nudged apart so their circles do not overlap.
  const minGap = 1.65;
  ...
  //  If the final group moves past the edge, the whole layout is pulled back.
...
function renderYearLabels() {
 ...
 function bindCuratedTimelineInteractions() {
  ...
  if (scrollArea) {
    ...
    // Empty timeline space clears the selected milestone.
...
function updateSelectedNodeState() {
  ...
  // Curated mode dims other milestones, while Detailed mode keeps every cluster list readable
...
// Leaving the Timeline restores all three views to their original state.
...
//  Archive List keeps every dataset record and separates dated from undated entries.
...
//A year jump moves the horizontal timeline or opens the matching archive group.
...
// The selected mode decides whether the page draws a visual timeline or the archive list
...
window.initialiseTimelineModule = initialiseTimelineModule;