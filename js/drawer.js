function initialiseDrawerModule() {
...
// Lookup tables are used first, with the full arrays kept as a fallback
...
function getMediaImages(media) {
  ...
  // Newer media records store each image with its own description
  ...
  // A direct image URL is used only when no local path is available
  if (
    !localUrls.length &&
...
// Related records are matched by ID or exact title.
// ext with no matching record is still shown, but without a link.
...
// Source records marked as internal are removed before the list is displayed.
...
function getMediaCaption(media) {
  ...
  // The media title is kept for alt text and internal checks, not shown as a caption
  return (
...
function normaliseLocalPathForCompare(
  ...
  try {
    ...
    // If decoding fails, the original path can still be compared.
  }
...
function getMediaSourceUrl(media) {
  // Use the media URL first, then check the linked source if needed.
...
window.initialiseDrawerModule = initialiseDrawerModule;